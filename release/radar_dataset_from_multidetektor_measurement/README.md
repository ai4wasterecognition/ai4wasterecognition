...
24 GHz FMCW radar dataset of plastic-waste samples in a fixed lab geometry, paired with numbered/named sample photographs. The radar measurements form the core; the photos make the set **multi-input ready** — radar frames and images of the same sample share one sample number (Nb), so the data can later be combined with image-based recognition.

**Version:** `radar_dataset_from_multidetektor_measurement_1.0.0`
...
- **43 sample photographs** with the same Nb numbering as the radar samples
...
| `checksums.sha256` | – | SHA-256 of every core data file |
| `photos/` | 43 | Sample photographs (`Nb.<n>_<description>_<view>.jpg`) |
| `aux/foto_catalog.csv` | 43 | Photo catalog: filename, nb_of_sample, description, view, has_radar_match |

## Sample photographs

Alongside the radar data there are photographs of the measured samples. **Each photo carries the same sample number (Nb) as the radar measurements**, so a radar frame and the photo(s) of the same sample line up 1:1 through `nb_of_sample`.

- The photos show both the **detailed composition of the waste** (the contamination objects spread out) and the **top-down view of the sample poured into the waste container** — the same scene the radar observes.
- This makes it possible to **compare results between radar sensing and video/image recognition** on identical samples.
- 33 photos correspond directly to a radar `nb_of_sample`; the remaining are detailed "spread-out" views of the contamination material (Nb 21, 51, 81, 161), one extra fill level (Nb 100), and 3 general measurement-setup slides.

Naming convention (from `data/Foto/`): `Nb.<sample_number>` — the sample number, identical to the radar sample numbering, followed by an English description of the photographed sample.

The photographs are included in this release under `photos/` and are also available as a zip archive: **xxx** (link TBD).

## Relation to the planned open video dataset

The repository will additionally host a separate **open video dataset** focused on recognising plastic and paper waste and their individual categories. That dataset is built over an extensive collection of images of these separated waste fractions. During the project it was used to train a neural network, which was then applied to recognise the photographs in this collection; the recognition results can be compared against the radar-based classification here. See [`docs/datasets/video_segmentation_dataset_v1/outputs.md`](../../docs/datasets/video_segmentation_dataset_v1/outputs.md).
...
base = "release/radar_dataset_from_multidetektor_measurement"
...
# Photo catalog (link a radar sample to its photos by nb_of_sample)
photos = pd.read_csv(f"{base}/aux/foto_catalog.csv")
print(photos[photos["nb_of_sample"] == 50])
```
...
```
data/multidetektor/meranie_23_04/<folder_ts>/FD/<Nb>_<ts>.txt   ← canonical raw radar
        │
        ├── ingest_raw_txt.py  (parse header + 17 bin rows × 4 channels)
        │       └── processed/.../df_bins.parquet
        └── (geometry constants from D4.31 report)

data/multidetektor/VysledkyPreStatistiku.xlsx                   ← label source
        └── build_sample_mapping.py → processed/.../sample_mapping.csv

data/Foto/Nb.<n>_<description>.jpg                              ← sample photos
        └── build_foto_catalog.py → photos/ + aux/foto_catalog.csv

bins + mapping → export_dataset.py → release/.../{parquet,npz,splits,...}
```

## Caveats

- **Single recording session per Nb.** Frames within one Nb are ~80 ms apart in a stationary scene → very high frame-to-frame correlation. Within-Nb time-ordered splits do not eliminate this. **Classification metrics on this v1 reflect memorisation of stable per-sample bin profiles, not true generalisation.** For honest generalisation estimates, collect a second recording session per Nb.
- **`measurement_distance_m`** (2.12 m) is a session-level constant, not a per-frame measurement.
- **`Category` typo fix.** The source xlsx originally contained `Plastic + Ai blinds`; the maintainer corrected this to `Plastic + Al blinds` directly in the xlsx before the export.

## Training & inference

Scripts (in [`scripts/radar_dataset_from_multidetektor_measurement/`](../../scripts/radar_dataset_from_multidetektor_measurement/)):
...
YAML configs in [`training/configs/radar_dataset_from_multidetektor_measurement/`](../../training/configs/radar_dataset_from_multidetektor_measurement/):
